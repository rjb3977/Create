...
public abstract class FluidManipulationBehaviour extends TileEntityBehaviour {
	...
	protected void playEffect(Level world, BlockPos pos, Fluid fluid, boolean fillSound) {
		...
		SoundEvent soundevent = SoundEvents.BUCKET_EMPTY;//fillSound ? fluid.getAttributes()
//			.getFillSound()
//			: fluid.getAttributes()
//				.getEmptySound();
		if (soundevent == null)
		...
		if (world instanceof ServerLevel)
			AllPackets.sendToNear((ServerLevel) world, splooshPos, 10, new FluidSplashPacket(splooshPos, new FluidStack(fluid, 1)));
	}
	...
	public void read(CompoundTag nbt, boolean clientPacket) {
		...
		if (nbt.contains("AffectedAreaFrom") && nbt.contains("AffectedAreaTo"))
			affectedArea = BoundingBoxUtil.from2BlockPos(NbtUtils.readBlockPos(nbt.getCompound("AffectedAreaFrom")),
