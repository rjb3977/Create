...
public class BeltSlicer {
	...
	public static InteractionResult useWrench(BlockState state, Level world, BlockPos pos, Player player,
		...
		if (hoveringEnd(state, hit)) {
			...
			world.setBlock(pos, Blocks.AIR.defaultBlockState(), 3 | Constants.BlockFlags.IS_MOVING);
	...
	public static InteractionResult useConnector(BlockState state, Level world, BlockPos pos, Player player,
		...
		if (!nextState.getMaterial()
			...
			if (!world.isClientSide) {
				...
				for (BlockPos blockPos : BeltBlock.getBeltChain(world, mergedController.getBlockPos())) {
					...
					belt.color = color;
					if (flipBelt)
						world.setBlock(blockPos, flipBelt(world.getBlockState(blockPos)), 3 | Constants.BlockFlags.IS_MOVING);
				}
		...
		if (!world.isClientSide) {
			...
			if (mergedController == null) {
				// Attach at end
				world.setBlock(next, state.setValue(BeltBlock.CASING, false), 3 | Constants.BlockFlags.IS_MOVING);
				BeltTileEntity segmentTE = BeltHelper.getSegmentTE(world, next);
				if (segmentTE != null)
